"""
Railway Backend for LogisticsManager - ИСПРАВЛЕННАЯ ВЕРСИЯ
"""
...
# Если Railway дал внутренний адрес, заменяем на публичный
if DATABASE_URL and "railway.internal" in DATABASE_URL:
    logger.warning("Detected internal Railway URL, using public URL instead")
    # Используем публичный URL из вашего Postgres-oYvQ
...
logger.info(f"Using database: {DATABASE_URL[:50]}..." if DATABASE_URL else "No database URL!")
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        logger.error(f"DATABASE_URL used: {DATABASE_URL[:30]}..." if DATABASE_URL else "None")
...
async def sync_data(request: SyncRequest, background_tasks: BackgroundTasks):
    ...
    db = SessionLocal()
    try:
        logger.info(f"Sync request from device: {request.DeviceId}, orders: {request.Orders.Count}")
